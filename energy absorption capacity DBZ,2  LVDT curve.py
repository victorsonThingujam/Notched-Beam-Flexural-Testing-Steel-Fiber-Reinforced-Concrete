...
A = (0.045032501, 2.699260101)   # Connect with B
B = (0.3, 0)                     # Next
C = (0.35, 0)                    # Then vertical up to D
D = (0.35, 9.749207076)          # Then curve back to A

# We'll build a polygon in the order: A -> B -> C -> D -> (curve from x=0.35 down to x=0.045) -> A

# ---------------------------------------------------------------------
# 4) Extract/Interpolate the curve from x=0.045... to x=0.35
# We'll sample N points in [0.045..0.35], including endpoints.
N = 50
x_curve = np.linspace(A[0], D[0], N)  # from 0.045032501 to 0.35
y_curve = np.interp(x_curve, x_all, y_all)

# The curve from x=0.35 back to x=0.045 means we want to reverse x_curve if we are
# building the polygon in a certain direction. We'll handle that carefully below.

# ---------------------------------------------------------------------
# 5) Build polygon points in order (clockwise or counterclockwise):
#    1. A
#    2. B
#    3. C
#    4. D
#    5. Then the curve from x=0.35 down to x=0.045
#       (except the endpoints D and A themselves to avoid duplication).
#    6. Back to A
...
# Now add the curve from x=0.35 down to x=0.045
# i.e. reversed sample from the N array, skipping the first and last if they are D or A
for i in range(N):
    idx = N - 1 - i
    xx = x_curve[idx]
    yy = y_curve[idx]
    # skip if (xx,yy) is exactly D or A
    # but let's do an approximate check:
...
# ---------------------------------------------------------------------
# 6) Compute area via shoelace formula
area = 0.5 * abs(
...
# (e) Optionally, highlight the curve sample in green
plt.plot(x_curve, y_curve, 'g-', lw=2, label="Curve x=0.045..0.35")

# Annotate the area on the plot
plt.text(0.2, 5, f"Area = {area:.3f} kN·mm", color='purple', fontsize=12)

plt.xlabel("Deflection (mm)")
plt.ylabel("Load (kN)")
plt.title("AreaD_{BZ,2}^f")
plt.grid(True)
plt.xlim(left=0)
plt.ylim(bottom=0)
plt.show()
